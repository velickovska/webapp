...
function locationInfo() {
    ...
    this.getStates = function(id) {
        ...
        var method = "post";
        var data = {};
        jQuery('.states').find("option:eq(0)").html("Ве молиме почекајте..");
        call.send(data, url, method, function(data) {
            jQuery('.states').find("option:eq(0)").html("Селектирај општина");
            if(data.tp == 1){
                if(data.hits > 1000)
                {
    ...
    this.getCountries = function() {
        ...
        var method = "post";
        var data = {};
        jQuery('.countries').find("option:eq(0)").html("Ве молиме почекајте..");
        call.send(data, url, method, function(data) {
            jQuery('.countries').find("option:eq(0)").html("Селектирај држава");
            //console.log(data);
            if(data.tp == 1){
                if(data.hits > 1000)
                {
